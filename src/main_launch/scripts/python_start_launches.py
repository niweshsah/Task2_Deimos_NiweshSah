...
def launch_navigation(location):
    # Launch turtlebot3_navigation with a map file
    subprocess.Popen(['gnome-terminal', '--', 'bash', '-c', f"roslaunch turtlebot3_navigation turtlebot3_navigation.launch map_file:={location}/map.yaml; exec bash"])
...
def main():
    ...
    if package_path:
        ...
        # Launch turtlebot3_gazebo which opens our bookstore world with turtlebot in it
        launch_gazebo(package_path)
        
        # Launch turtlebot3_navigation with a map file
        map_location = os.path.expanduser("~")#If your file is saved somewhere else, then use that path (our file is at home directory)
        launch_navigation(map_location) 
